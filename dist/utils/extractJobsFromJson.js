...
const TITLE_HINT = /\b(designer|design|artist|writer|producer|engineer|developer|manager|director|specialist|analyst|research|intern|architect|lead)\b/i;
...
function toRawJob(record, baseUrl) {
    ...
    if (!url)
        return null;
    if (!URL_HINT.test(url) && !TITLE_HINT.test(title))
        return null;
    return {
        title,
        url,
...
function extractFromJsonLdJobPosting(data, baseUrl, jobs) {
    if (!data || typeof data !== "object")
        return;
    if (Array.isArray(data)) {
        for (const item of data)
            extractFromJsonLdJobPosting(item, baseUrl, jobs);
        return;
    }
    const record = data;
    ...
    if (type === "jobposting") {
        ...
        const url = (typeof record.url === "string" && safeAbsoluteUrl(record.url, baseUrl)) ||
            (typeof record.sameAs === "string" && safeAbsoluteUrl(record.sameAs, baseUrl)) ||
            null;
        if (title && url) {
            const locationObj = record.jobLocation;
...
export function extractJobsFromJson(data, baseUrl) {
    const jobs = [];
