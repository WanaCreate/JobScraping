...
const LOCATION_NOISE_PATTERN = /\b(responsibilit|qualification|benefit|apply for this job|job alert|equal opportunity|privacy notice|what you'll|you will|our team|interview|voluntary self|screening question|portfolio|resume|autofill)\b/i;
let playwrightActiveFetches = 0;
const PLAYWRIGHT_MAX_PARALLEL_FETCHES = 2;
function delay(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
function normalizeJobUrlForRetry(rawUrl) {
    try {
...
function isKnownAtsHost(url) {
    try {
        const host = new URL(url).hostname.toLowerCase();
        return /(greenhouse\.io|lever\.co|smartrecruiters\.com|myworkdayjobs\.com|icims\.com|ashbyhq\.com|phenompeople\.com|workdayjobs\.com)/i.test(host);
    }
    catch {
        return false;
    }
...
async function fetchBestJobPage(rawUrl) {
...
function normalizeRootWebsite(url) {
    try {
...
function extractJsonLdJobPosting(html) {
    ...
    while (queue.length > 0) {
        ...
        if (/jobposting/i.test(typeValue))
            return record;
...
function cleanDescriptionWithTitle(description, title) {
    let d = cleanText(description);
    const t = cleanText(title);
    d = d.replace(/\bBack\s+Apply\s+Share\s+(?:Facebook|X|Twitter|LinkedIn|Email)(?:\s+(?:Facebook|X|Twitter|LinkedIn|Email))*\b/gi, " ");
    ...
    d = d.replace(/^\s*(new\s+)?(apply\s+|share\s+|facebook\s+|x\s+|twitter\s+|linkedin\s+|email\s+)+/i, "");
    d = d.replace(/^\s*description\s*/i, "");
    return cleanText(d);
}
function extractDescription(html, jsonLdJob) {
    if (jsonLdJob) {
        const jsonDescription = nonEmpty(String(jsonLdJob.description ?? ""));
        if (jsonDescription && jsonDescription.length > 80)
            return sanitizeDescription(jsonDescription);
    }
    const $ = load(html);
    let best = "";
...
function isLikelyLocationPhrase(raw) {
    const value = cleanText(raw);
    if (!value || value.length > 90)
        return false;
    if (LOCATION_NOISE_PATTERN.test(value))
        return false;
    if (/[{}<>]/.test(value))
        return false;
    const wordCount = value.split(/\s+/).length;
    if (wordCount > 8 && !/\bremote\b/i.test(value))
        return false;
    return true;
}
function normalizeRemoteRegion(raw) {
...
function parseDelimitedLocation(raw) {
    const base = emptyLocation();
    const cleaned = raw
        .replace(/[|]/g, ",")
        .replace(/[\u2022]/g, ",")
...
function parseLocationString(rawInput) {
    const raw = cleanText(rawInput);
    if (!raw || /^not specified$/i.test(raw) || /^n\/?a$/i.test(raw))
        return null;
    ...
    if (!isLikelyLocationPhrase(candidate))
        return null;
...
function extractLocationFromJsonLd(jsonLdJob) {
...
function extractLocation(seedLocation, jsonLdJob, description) {
    ...
    const descriptionLocationMatch = description.match(/\b(location|based in|office location|work location)\s*[:\-]\s*([A-Za-z .'-]{3,80}(?:,\s*[A-Za-z .'-]{2,40}){0,3})/i);
...
function inferJobType(title, description, jsonLdJob) {
    const combined = `${title} ${description}`.toLowerCase();
    const employmentType = cleanText(String(jsonLdJob?.employmentType ?? "")).toLowerCase();
    const fromJson = `${employmentType} ${combined}`;
    if (/\b(intern|internship|temporary|fixed[- ]term|contract)\b/.test(fromJson))
        return "GIG";
    if (/\b(part[- ]?time)\b/.test(fromJson))
        return "PARTTIME";
    if (/\b(freelance|contractor)\b/.test(fromJson))
        return "FREELANCE";
    return "FULLTIME";
}
function inferWorkType(location, title, description) {
...
function normalizePeriod(input) {
    ...
    if (text.includes("year") || text.includes("annual"))
        return "ANNUAL";
    if (text.includes("one") || text.includes("fixed"))
        return "ONE_TIME";
    return null;
}
...
function extractSalary(description, jsonLdJob) {
    const fromJson = jsonLdJob?.baseSalary;
    if (fromJson && typeof fromJson === "object") {
        const currency = cleanText(String(fromJson.currency ?? "USD")) || "USD";
        const value = fromJson.value;
    ...
    const rangeMatch = description.match(/([$ۣ])\s?([0-9]{2,3}(?:,[0-9]{3})*(?:\.[0-9]+)?[kKmM]?)\s*(?:-|to)\s*([$ۣ])?\s?([0-9]{2,3}(?:,[0-9]{3})*(?:\.[0-9]+)?[kKmM]?)\s*(?:per|\/)??\s*(hour|day|week|month|year|annual)?/i);
    if (!rangeMatch)
        return null;
...
function extractSkills(title, description) {
    ...
    const skillPhrasePattern = /\b(proficiency in|experience with|expertise in|strong knowledge of|hands[- ]on with|skilled in)\s+([^.;:\n]{3,160})/gi;
...
function extractDeadline(description, jsonLdJob) {
    ...
    const deadlineMatch = description.match(/\b(apply by|application deadline|deadline)\s*[:\-]?\s*([A-Za-z]{3,9}\s+\d{1,2},?\s+\d{4})/i);
    if (!deadlineMatch?.[2])
        return null;
    const parsed = new Date(deadlineMatch[2]);
    return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString();
}
function extractNumberOfPositions(description) {
...
function extractCompany(seed, finalUrl, jsonLdJob, email) {
    const org = jsonLdJob?.hiringOrganization;
    const orgRecord = org && typeof org === "object" ? org : null;
    const derivedName = nonEmpty(seed.company) ??
        nonEmpty(orgRecord ? String(orgRecord.name ?? "") : "") ??
        deriveCompanyNameFromUrl(finalUrl);
    const website = nonEmpty(orgRecord ? String(orgRecord.sameAs ?? orgRecord.url ?? "") : "") ??
        normalizeRootWebsite(finalUrl);
    if (!derivedName && !website && !email)
        return null;
    return {
        name: derivedName,
        website,
...
function extractTitle(html, seedTitle, jsonLdJob) {
...
export async function enrichJobFromUrl(params) {
    ...
    if (!passesGate) {
        ...
        const weakDescription = description.length < 60 ||
            /\b(apply for this job|create a job alert|autofill|voluntary self-identification|resume\/cv)\b/i.test(description);
