...
function reviewTitle(job, issues) {
...
function reviewDescription(job, issues) {
...
function reviewJobType(job, issues) {
...
function inferJobTypeFromText(text) {
    // Strong signal patterns (word boundaries to avoid false matches)
    if (/\b(internship|intern\b)/i.test(text))
        return "GIG";
    if (/\b(contract|contractor|fixed[- ]?term|temporary)\b/i.test(text))
        return "GIG";
    if (/\bpart[- ]?time\b/i.test(text))
        return "PARTTIME";
    if (/\bfreelance\b/i.test(text))
        return "FREELANCE";
    if (/\bfull[- ]?time\b/i.test(text))
        return "FULLTIME";
    return null;
}
function reviewLocationAndWorkType(job, issues) {
    let wasFixed = false;
    // ── Location cleanup ──
    if (job.location) {
...
function inferWorkTypeFromText(text) {
    if (/\b(remote|work\s*from\s*home|distributed|anywhere)\b/i.test(text))
        return "REMOTE";
    if (/\b(hybrid|flexible\s*office)\b/i.test(text))
        return "HYBRID";
    if (/\b(on[- ]?site|onsite|in[- ]office|in[- ]person|studio[- ]based)\b/i.test(text))
        return "ONSITE";
    return null;
}
function reviewCompanyName(job, issues) {
    if (!job.company?.name)
        return null;
...
function extractCompanyFromJobLink(url) {
    try {
...
function reviewDescriptionFormatting(job, issues) {
    let wasFixed = false;
...
function reviewTitleContent(job, issues) {
    const title = job.title.trim();
...
async function callAiReview(apiKey, input) {
...
function needsAiReview(job) {
    const plainDesc = stripHtml(job.description);
    // Short but not empty descriptions — can't tell if relevant without AI
    if (plainDesc.length >= 30 && plainDesc.length < 200)
        return true;
    // Title looks suspicious (all caps, contains weird chars, very generic)
    if (/^[A-Z\s]{10,}$/.test(job.title))
        return true;
    if (/^(job|position|role|opening|vacancy|opportunity)\s*$/i.test(job.title))
        return true;
    // Title is a careers-page heading — AI should extract real title from description
    if (/^.{2,30}\s+(careers?|jobs?|openings?|opportunities)$/i.test(job.title))
        return true;
    // Company name is generic — AI may be able to find the real name
    if (job.company?.name && GENERIC_COMPANY_NAMES.has(job.company.name.toLowerCase()))
        return true;
    // Unformatted blob description — AI can help restructure
    const hasHtml = /<[^>]+>/.test(job.description);
    const hasNewlines = /\n/.test(job.description);
    if (!hasHtml && !hasNewlines && plainDesc.length > 500)
        return true;
    ...
    if (plainDesc.length >= 200 && !jobKeywords.test(plainDesc))
        return true;
    return false;
}
// ─── Main Review Function ────────────────────────────────────────
export async function reviewJobs(jobs, options = {}) {
    const apiKey = process.env.ANTHROPIC_API_KEY ?? null;
    const skipAi = options.skipAiReview || !apiKey;
    const stats = { total: jobs.length, ok: 0, fixed: 0, dropped: 0, issues: [] };
    const reviewed = [];
    const needsAiJobs = [];
    ...
    if (needsAiJobs.length > 0 && !skipAi) {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (!result.isValidJob) {
                        ...
                        reviewed[index] = null; // Mark for removal
                        aiDropped++;
                        const issue = { field: "all", problem: `AI: not a valid job posting — ${result.reason ?? "no reason"}`, action: "dropped" };
                    ...
                    if (!result.descriptionRelevant) {
                        ...
                        reviewed[index] = null;
                        aiDropped++;
                        const issue = { field: "description", problem: `AI: description not relevant to job — ${result.reason ?? ""}`, action: "dropped" };
                        stats.issues.push(issue);
                    ...
                    if (!result.titleOk && result.fixedTitle && result.fixedTitle.length >= 3) {
                        const issue = { field: "title", problem: "AI: title doesn't match description", action: "fixed", before: job.title, after: result.fixedTitle };
                        stats.issues.push(issue);
                    ...
                    if (result.jobTypeSuggestion && result.jobTypeSuggestion !== job.jobType) {
                        const issue = { field: "jobType", problem: "AI: jobType mismatch", action: "fixed", before: job.jobType, after: result.jobTypeSuggestion };
                        stats.issues.push(issue);
                    ...
                    if (result.workTypeSuggestion !== undefined && result.workTypeSuggestion !== job.workType) {
                        const issue = { field: "workType", problem: "AI: workType mismatch", action: "fixed", before: job.workType ?? "null", after: result.workTypeSuggestion ?? "null" };
                        stats.issues.push(issue);
                    ...
                    if (result.companyNameSuggestion && result.companyNameSuggestion !== job.company?.name) {
                        const issue = { field: "company", problem: "AI: company name incorrect", action: "fixed", before: job.company?.name ?? "null", after: result.companyNameSuggestion };
                        stats.issues.push(issue);
                        if (!job.company) {
        ...
        const filtered = reviewed.filter((j) => j !== null);
 ...
 */
export function generatePipelineInstructions(stats) {
    ...
    const instructions = sorted.map(([key, data]) => {
...
function normalizePattern(problem) {
    return problem
...
function buildInstruction(field, pattern, action, count, examples) {
