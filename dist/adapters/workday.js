...
function buildWorkdayCandidates(sourceUrl, tenant, endpoints) {
    const candidates = [...endpoints];
    if (tenant) {
        const match = sourceUrl.match(/https?:\/\/([a-z0-9.-]*myworkdayjobs\.com)/i);
        if (match?.[1]) {
            const host = match[1];
            candidates.push(`https://${host}/wday/cxs/${tenant}/External/jobs`, `https://${host}/wday/cxs/${tenant}/Careers/jobs`);
        }
    }
    return unique(candidates);
}
function buildPostingUrl(endpoint, posting) {
