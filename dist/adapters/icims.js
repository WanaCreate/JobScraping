...
function extractJobsFromHtml(html, pageUrl) {
    ...
    $("a").each((_, element) => {
        ...
        if (!href)
            return;
        if (!/\/jobs\/\d+/i.test(href) || !/\/job/i.test(href))
            return;
        ...
        const locationMatch = rowText.match(/\b(remote|[a-z .'-]+,\s*[a-z .'-]+(?:,\s*[a-z .'-]+)?)\b/i);
        jobs.push({
            title,
            url: absolute,
...
function buildIcimsEndpoints(tenant, endpoints) {
...
async function scrapeIcimsSearchEndpoint(endpoint) {
    const jobs = [];
    const maxPages = 50;
    let emptyStreak = 0;
    for (let page = 0; page < maxPages; page++) {
        ...
        const response = await http.get(pagedUrl, { responseType: "text" });
