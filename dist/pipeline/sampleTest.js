...
// ─── Run ─────────────────────────────────────────────────────────
const jobs = fixtures.map(f => buildJobFromHeuristics(f.html, f.url, f.seedTitle, f.jsonLd, BASE_UID));
const checks = [];
let failCount = 0;
function check(row, field, expected, actual) {
...
jobs.forEach((job, i) => {
    ...
    if (job.salary?.min != null)
        console.log(`       salary=${job.salary.min}-${job.salary.max} ${job.salary.currency} ${job.salary.period}`);
    if (job.company?.name)
        console.log(`       company=${job.company.name}  companyEmail=${job.company.email ?? "(none)"}`);
    if (job.deadline)
