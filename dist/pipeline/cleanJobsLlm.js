...
const DEFAULT_INPUT = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest\\results_jobs_api.json";
const DEFAULT_OUTPUT_JSON = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest\\results_jobs_api.llm.json";
const DEFAULT_OUTPUT_CSV = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest\\results_jobs_api.llm.csv";
const DEFAULT_OUTPUT_REPORT = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest\\results_jobs_quality_report.llm.json";
const PLACEHOLDER_DESC = "For job details, click apply.";
function parseArgs(argv) {
    const positional = argv.filter((x, i) => {
        if (x.startsWith("--"))
            return false;
        if (i > 0 && argv[i - 1].startsWith("--"))
            return false;
        return true;
    });
    const get = (flag) => {
...
function cleanupDescription(description) {
    ...
    d = d.replace(/^\s*(?:now\s*)?(?:back\s*to\s*jobs?|back\s*to\s*search\s*results?|search\s*results?)\s*[:|>\-–—]*\s*/i, "");
    d = d.replace(/(?:take\s*20%\s*off any new website plan|use code at checkout|offer termsclose|discount applies to the first payment|payments through in-app pay)[\s\S]{0,500}/i, " ");
    d = d.replace(/(CXT\.CLIENT_SIDE_METRICS|CXT\.ANALYTICS|#\$\(['"][^'"]+['"]\)\.click|share this job)[\s\S]{0,900}/gi, " ");
    d = d.replace(/\s+/g, " ").trim();
    return d || PLACEHOLDER_DESC;
}
function hasJobIdSignal(url) {
    try {
        const u = new URL(url);
        if (/\/\d{5,}(\/|$)/.test(u.pathname))
            return true;
...
function flagJob(job, index) {
    ...
    if (/back\s*to\s*jobs?|back\s*to\s*search\s*results?|search\s*results?/i.test(desc))
        reasons.push("breadcrumb_prefix");
    ...
    if (isLikelyListingPage(url) && !hasJobIdSignal(url))
        reasons.push("listing_page_url");
    if (/^open positions?\s*-\s*/i.test(title))
        reasons.push("listing_prefix_title");
...
function parseDecision(raw) {
    ...
    try {
        ...
        if (action !== "keep" && action !== "remove" && action !== "rewrite")
            return null;
        ...
        return {
            ...
            cleaned_description: typeof parsed.cleaned_description === "string" ? parsed.cleaned_description : null,
...
async function callLlmDecision(flagged, options) {
    if (options.dryRun) {
        return {
            action: "keep",
            cleaned_title: null,
    ...
    const systemPrompt = "You are a strict job-data cleaner. Return JSON only. Never invent job facts. Keep jobLink/company unchanged. If unsure, keep.";
...
function toCsv(jobs) {
    const header = "title,description,jobType,deadline,keywords,skills,jobLink,hiringTeam,workType,workEmail,numberOfPositions,company,companyWebsite,companyLogo,companyEmail,locationName,formattedAddress,city,state,country,latitude,longitude,salaryMin,salaryMax,salaryCurrency,salaryPeriod";
...
function dedupeByCanonicalLink(jobs) {
    const seen = new Set();
    const out = [];
