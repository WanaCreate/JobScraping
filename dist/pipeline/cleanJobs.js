...
import { canonicalizeUrl, stepARemoveNoise, stepBRemoveNonCreative, stepCNormalizeTitle, stepDNormalizeDescription, stepENormalizeFields, stepFDedupe, } from "../utils/jobCleaner.js";
const API_FILE = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest\\results_jobs_api.json";
const ENRICHED_FILE = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest\\results_jobs_enriched.json";
const OUTPUT_DIR = "C:\\Users\\vyash\\Desktop\\Business\\Wana\\_Code\\JobScraping\\outputs\\api-ready\\latest";
...
const CSV_HEADER = "title,description,jobType,deadline,keywords,skills,jobLink,hiringTeam,workType,workEmail,numberOfPositions,company,companyWebsite,companyLogo,companyEmail,locationName,formattedAddress,city,state,country,latitude,longitude,salaryMin,salaryMax,salaryCurrency,salaryPeriod";
async function loadInputs() {
    const [apiBuf, enrichedBuf] = await Promise.all([
        readFile(API_FILE, "utf-8"),
        readFile(ENRICHED_FILE, "utf-8"),
    ]);
    const jobs = JSON.parse(apiBuf);
    const enriched = JSON.parse(enrichedBuf);
    const creativeScoreByLink = new Map();
...
function runPipeline(jobs, creativeScoreByLink, ctx) {
    let current = jobs;
    // Step A: Remove noise
    ...
    current = current.map((j) => {
        ...
        const description = stepDNormalizeDescription({ ...j, title, description: j.description }, ctx, effectiveLink);
...
async function main() {
    ...
    // Final checks
    const hasSkipTitle = cleaned.some((j) => (j.title ?? "").toLowerCase() === "skip to main content");
    const hasEmptyDesc = cleaned.some((j) => !(j.description ?? "").trim());
    if (hasSkipTitle || hasEmptyDesc) {
        throw new Error(`Final check failed: skipTitle=${hasSkipTitle} emptyDesc=${hasEmptyDesc}`);
    }
    const report = {
    ...
    try {
        ...
        const fallbackCsv = path.join(OUTPUT_DIR, `results_jobs_api.cleaned.${Date.now()}.csv`);
