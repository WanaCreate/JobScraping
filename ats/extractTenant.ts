import type { ATS, TenantInfo } from "../types.js";
...
function parseFirstMatch(text: string, patterns: RegExp[]): string | null {
...
function extractWorkdayEndpoints(html: string, baseUrl: string): string[] {
...
function extractIcimsSearchEndpoint(html: string, finalUrl: string): string[] {
  const direct = html.match(/https?:\/\/[^"'\s<>]*icims\.com\/jobs\/search[^"'\s<>]*/gi) ?? [];
  if (direct.length > 0) return unique(direct.map((x) => x.replace(/\\u0026/g, "&")));
...
export function extractTenant(html: string, finalUrl: string, ats: ATS): TenantInfo {
  ...
  if (ats === "icims") {
    const tenant =
      parseFirstMatch(joined, [/\/\/([a-z0-9-]+)\.icims\.com/i, /clientid=([a-z0-9-]+)/i]) ?? null;
  ...
  if (ats === "phenom") {
    const tenant =
      parseFirstMatch(html, [/"refNum"\s*:\s*"([A-Z0-9_-]+)"/i, /content-us\.phenompeople\.com\/api\/([A-Z0-9_-]+)\//i]) ??
      null;

    const widgetEndpoint =
      parseFirstMatch(html, [/"widgetApiEndpoint"\s*:\s*"([^"]+)"/i, /https?:\/\/[^"'\s<>]+\/widgets/i]) ?? null;

    if (widgetEndpoint) endpoints.push(widgetEndpoint);
    return { tenant, metadata, endpoints: unique(endpoints) };
  }
