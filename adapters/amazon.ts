...
function formatLocation(posting: AmazonPosting): string | null {
  const parts = [posting.city, posting.state, posting.country].filter(Boolean);
  if (parts.length > 0) return parts.join(", ");
  return posting.location ?? null;
}

export async function scrapeAmazon(sourceUrl: string): Promise<RawJob[]> {
  ...
  while (page < maxPages) {
    const response = await http.get<AmazonResponse>(searchUrl, {
      params: {
        offset,
        result_limit: limit
      }
