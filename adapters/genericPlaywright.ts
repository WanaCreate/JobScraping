...
const PAGE_HINT =
  /(\/(jobs|career|careers|search|open-positions|opportunit)\b|\/c\/[a-z0-9-]+-jobs\b|work-with-us|join-us)/i;
const NOISE_TITLE =
  /\b(home|about|learn more|privacy|terms|cookie|login|sign in|register|filter|location|all jobs|search|faq)\b/i;
...
async function gotoWithRetries(page: import("playwright").Page, url: string): Promise<boolean> {
  const waitUntilOptions: Array<"domcontentloaded" | "load" | "networkidle"> = [
    "domcontentloaded",
    "load"
  ];
...
export async function scrapeGenericPlaywright(
  url: string,
  options?: { seedUrls?: string[]; maxPages?: number }
): Promise<RawJob[]> {
  ...
  try {
    ...
    if (options?.seedUrls?.length) candidatePages.push(...options.seedUrls);
      ...
      try {
        ...
        for (const anchor of anchors) {
          ...
          if (PAGE_HINT.test(absolute) && isAllowedCrawlUrl(absolute, url) && !visited.has(absolute)) {
            if (!queue.includes(absolute)) queue.push(absolute);
          }

          if (!title || title.length < 4 || title.length > 150) continue;
          if (NOISE_TITLE.test(title)) continue;
          if (!URL_HINT.test(absolute) && !TITLE_HINT.test(title)) continue;
        ...
        const embeddedJson = await page.$$eval(
          "script[type='application/ld+json'], script#__NEXT_DATA__",
          (elements) => elements.map((element) => element.textContent || "").filter(Boolean)
        );
