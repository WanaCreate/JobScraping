...
function needsPlaywright(url: string): boolean {
  try {
    ...
    if (PLAYWRIGHT_DOMAINS.some(domain => host.includes(domain))) return true;

    // careers.X.com and jobs.X.com often embed JS-rendered ATS widgets
    const parts = host.split(".");
    if (parts.length >= 3 && JS_RENDERED_SUBDOMAINS.has(parts[0])) return true;
...
function stripApplyPath(url: string): string {
  try {
    const parsed = new URL(url);
...
export async function fetchJobPage(rawUrl: string): Promise<{ html: string; finalUrl: string } | null> {
  ...
  let httpResult: { html: string; finalUrl: string } | null = null;
  try {
