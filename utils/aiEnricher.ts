...
function truncatePageText(text: string, maxChars = 12000): string {
...
function parseAiResponse(raw: string): AiParsedJob | null {
  let text = raw.trim();
  ...
  try {
    ...
    const jobType = (JOB_TYPE_VALUES.has(rawJobType) ? rawJobType : "FULLTIME") as JobTypeValue;
    const rawWorkType = parsed.workType ? String(parsed.workType).toUpperCase() : null;
    const workType = rawWorkType && WORK_TYPE_VALUES.has(rawWorkType) ? rawWorkType as WorkTypeValue : null;
    ...
    let salary: ApiSalary | null = null;
    if (parsed.salary && typeof parsed.salary === "object") {
      const sal = parsed.salary as Record<string, unknown>;
      ...
      const period = SALARY_PERIOD_VALUES.has(rawPeriod) ? rawPeriod as SalaryPeriodValue : null;
    ...
    if (parsed.company && typeof parsed.company === "object") {
      const comp = parsed.company as Record<string, unknown>;
    ...
    const keywords = Array.isArray(parsed.keywords)
      ? parsed.keywords.filter((k): k is string => typeof k === "string" && k.trim().length > 0).map(k => k.trim().toLowerCase())
      : [];

    const skills = Array.isArray(parsed.skills)
      ? parsed.skills.filter((s): s is string => typeof s === "string" && s.trim().length > 0).map(s => s.trim().toLowerCase().replace(/\s+/g, "_"))
      : [];
...
export async function enrichWithAi(input: AiEnrichInput): Promise<ApiCreateJobRequest | null> {
...
export function needsAiEnrichment(
  ...
  if (missingSkills || missingSalary) return true;
  ...
  if (jsonLdJob) {
    ...
    if (jsonDesc.length >= 300) return false;
  }
