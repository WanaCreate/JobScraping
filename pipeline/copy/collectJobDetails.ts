...
function parseCliOptions(): CliOptions {
  ...
  const maxJobsRaw = getArg("--maxJobs") ?? positional[1] ?? null;
  const concurrency = parseNumber(getArg("--concurrency") ?? positional[2] ?? null, 10);
  const hiringTeamPositional = positional[3] ?? null;
  ...
  const output = getArg("--output") ?? positional[4] ?? `${latestDir}/results_jobs_enriched.json`;
  const apiOutput = getArg("--apiOutput") ?? positional[5] ?? `${latestDir}/results_jobs_api.json`;
  const csvOutput = getArg("--csvOutput") ?? positional[6] ?? `${latestDir}/results_jobs_api.csv`;
  const reportOutput =
    getArg("--reportOutput") ?? positional[7] ?? `${latestDir}/results_jobs_quality_report.json`;
...
async function main(): Promise<void> {
  ...
  const enrichedRaw = await runWithConcurrency(scopedJobs, options.concurrency, async (job) =>
    enrichJobFromUrl({
      seed: job,
  ...
  const enriched = enrichedRaw.filter((record): record is EnrichedJobRecord => record !== null);
