 ...
 *
 * Usage:
 *   ANTHROPIC_API_KEY=sk-... npx tsx pipeline/enrichFromCsv.v2.ts [options]
 *
 * Options:
...
function extractJsonLdJobPosting(html: string): Record<string, unknown> | null {
  ...
  while (queue.length > 0) {
    ...
    if (/jobposting/i.test(typeStr)) return record;
...
function scoreDescriptionCandidate(htmlFragment: string, priorityBoost: number): number {
  const plain = stripHtmlToPlain(htmlFragment);
  if (plain.length < 60) return -1;

  let score = Math.min(plain.length, 20000) + priorityBoost;
  if (/<(p|ul|ol|li|h2|h3|h4|br)\b/i.test(htmlFragment)) score += 140;
  if (/\b(responsibilit|qualification|requirement|what you'll|you will|about the role|benefits)\b/i.test(plain)) score += 120;
  if (/\b(cookie|privacy policy|sign in|log in)\b/i.test(plain)) score -= 800;
  if (DESCRIPTION_FORM_NOISE.some(token => plain.toLowerCase().includes(token))) score -= 900;
  if (plain.length > 16000) score -= 200;
  return score;
}
...
function extractDescriptionFromHtml(html: string, jsonLd: Record<string, unknown> | null): string {
...
function extractLocationFromJsonLd(jsonLd: Record<string, unknown>): ApiLocation | null {
  ...
  if (country.length <= 3) country = COUNTRY_CODES[country.toUpperCase()] ?? country;
...
function extractSalaryFromJsonLd(jsonLd: Record<string, unknown>): ApiSalary | null {
  ...
  const value = sal.value as Record<string, unknown> | undefined;
  ...
  const periodMap: Record<string, SalaryPeriodValue> = {
...
function extractSkillsFromText(title: string, description: string): string[] {
...
function extractWorkEmail(html: string, description: string): string | null {
...
function extractSalaryFromText(description: string): ApiSalary | null {
  ...
  let period: SalaryPeriodValue | null = periodMap[periodStr] ?? null;

  // Infer period from magnitude if not specified
  if (!period) {
    if (min >= 10000) period = "ANNUAL";
    else if (min >= 1000) period = "MONTHLY";
    else if (min >= 100) period = "DAILY";
    else period = "HOURLY";
  }
...
function inferJobTypeFromJsonLd(jsonLd: Record<string, unknown>): JobTypeValue {
  const raw = String(jsonLd.employmentType ?? "").toLowerCase();
  if (/intern|contract|temporary|fixed/i.test(raw)) return "GIG";
  if (/part[- ]?time/i.test(raw)) return "PARTTIME";
...
function extractCompanyFromJsonLd(jsonLd: Record<string, unknown>, fallbackUrl: string): ApiCompany | null {
  ...
  if (org && typeof org === "object") {
    ...
    if (name) return { name, website: website || null, logo: null, email: null };
  }
...
function extractTitleFromPage(html: string, seedTitle: string, jsonLd: Record<string, unknown> | null): string {
  if (jsonLd) {
    const t = cleanText(String(jsonLd.title ?? ""));
    if (t && !isCareersPageTitle(t)) return t;
  }

  const $ = load(html);
  const h1 = cleanText($("h1").first().text());
  if (h1 && h1.length > 3 && h1.length < 200 && !isCareersPageTitle(h1)) return h1;
...
function buildJobFromHeuristics(
  ...
  const descriptionText = extractDescriptionFromHtml(html, jsonLd);
  const description = descriptionHtml || descriptionText || "For job details, click apply.";
  const descriptionPlain = stripHtmlToPlain(description);
  ...
  const workType = inferWorkType(title, descriptionPlain, location);

  const jsonSkills = jsonLd ? extractSkillsFromJsonLd(jsonLd) : [];
  const textSkills = extractSkillsFromText(title, descriptionPlain);
  const skills = dedupeStrings([...jsonSkills, ...textSkills]);

  const jsonKeywords = jsonLd ? extractKeywordsFromJsonLd(jsonLd) : [];
  const textKeywords = extractKeywordsFromText(title, descriptionPlain);
  const keywords = dedupeStrings([...jsonKeywords, ...textKeywords]);

  const workEmail = extractWorkEmail(html, descriptionPlain);
  ...
  return {
    ...
    allowEmailApplications: Boolean(workEmail || company?.email),
  };
}

function scoreDescriptionQuality(desc: string): number {
  const plain = stripHtmlToPlain(desc);
  let score = plain.length;
  if (hasHtmlTags(desc)) score += 300;
  if (isPlaceholderDescription(desc)) score -= 1000;
  if (DESCRIPTION_FORM_NOISE.some(token => plain.toLowerCase().includes(token))) score -= 700;
  return score;
}

function chooseBestDescription(heuristicDesc: string, aiDesc: string): string {
  const heuristicScore = scoreDescriptionQuality(heuristicDesc);
  const aiScore = scoreDescriptionQuality(aiDesc);
  if (aiScore > heuristicScore + 250) return aiDesc;
  return heuristicDesc;
}

function mergeWithAi(heuristic: ApiCreateJobRequest, aiResult: ApiCreateJobRequest): ApiCreateJobRequest {
  const mergedKeywords = dedupeStrings([...(heuristic.keywords ?? []), ...(aiResult.keywords ?? [])]);
  const mergedSkills = dedupeStrings([...(heuristic.skills ?? []), ...(aiResult.skills ?? [])]);

  const mergedCompany = heuristic.company?.name
    ? { ...(aiResult.company ?? {}), ...heuristic.company }
    : (aiResult.company ?? heuristic.company ?? null);

  const mergedCompanyEmail = heuristic.company?.email
    ?? aiResult.company?.email
    ?? heuristic.workEmail
    ?? aiResult.workEmail
    ?? null;

  if (mergedCompany) {
    mergedCompany.email = mergedCompanyEmail;
  }

  const workEmail = heuristic.workEmail ?? aiResult.workEmail ?? mergedCompany?.email ?? null;
  ...
  return {
    ...
    description: chooseBestDescription(heuristic.description, aiResult.description),
    ...
    screeningQuestions: [],
    screeningRequired: false,
    allowEmailApplications: Boolean(workEmail || mergedCompany?.email),
  };
}
...
function dedupeJobs(jobs: ApiCreateJobRequest[]): ApiCreateJobRequest[] {
  const seen = new Map<string, ApiCreateJobRequest>();
...
async function main(): Promise<void> {
  ...
  // ── Phase 1: Fetch pages and extract with heuristics ───────────
  ...
  interface FetchResult {
    ...
    finalUrl: string;
    jsonLd: Record<string, unknown> | null;
    heuristicJob: ApiCreateJobRequest | null;
    fetchError: boolean;
  }
  ...
  const fetchResults = await runConcurrent(rows, options.concurrency, async (row, index) => {
    ...
    try {
      ...
      if (result.jsonLd) jsonLdCount++;

      result.heuristicJob = buildJobFromHeuristics(
        fetched.html, fetched.finalUrl, row.title, result.jsonLd, options.hiringTeamUid,
      );
  ...
  const allJobs: ApiCreateJobRequest[] = [];
  const needsAi: Array<{ index: number; fetchResult: FetchResult }> = [];

  for (let i = 0; i < fetchResults.length; i++) {
    const r = fetchResults[i];
    if (r.fetchError || !r.heuristicJob) continue;
  ...
  for (const job of reviewedJobs) {
    if (!isPlaceholderDescription(job.description)) withDescription++;
    if (job.location) withLocation++;
    if (job.salary) withSalary++;
    if (job.company?.name) withCompany++;
    if (job.skills && job.skills.length > 0) withSkills++;
  }
